---
name: queue-dev
description: Watch the queue board on the development server (port 3334) and work through tasks autonomously. Use this when starting a coding session with the dev queue board.
---
...
1. **If no arguments provided** (just `/queue-dev` with nothing after):
   - Call `queue_list_projects` to get available projects
   - Display the result to the user and stop

2. **If arguments start with `plan`** but no project ID after it (just `/queue-dev plan`):
   - Call `queue_list_projects` to get available projects
...
- If arguments start with `plan` (e.g., `/queue-dev plan kbn-xxxx`), follow **Planning Mode** below
- Otherwise (e.g., `/queue-dev kbn-xxxx`), follow **Work Mode** (Main Loop) below
...
Work through tasks autonomously on the DEVELOPMENT server (port 3334).
...
This uses the DEV MCP server (claude-queue-dev) on port 3334.
