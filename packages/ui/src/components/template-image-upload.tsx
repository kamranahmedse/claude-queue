...
export function TemplateImageUpload(props: TemplateImageUploadProps) {
  const { templateId, disabled = false } = props;

  const [isDragging, setIsDragging] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const { data: attachments = [] } = useQuery(listTemplateAttachmentsOptions(templateId));
  const uploadAttachment = useUploadTemplateAttachment(templateId);
  const deleteAttachment = useDeleteTemplateAttachment(templateId);
